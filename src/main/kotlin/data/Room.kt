...
import com.jesushz.other.getRandomWords
import com.jesushz.other.matchesWord
import com.jesushz.other.transformToUnderscores
import com.jesushz.other.words
...
class Room(
    ...
    private fun newRound() {
        ...
        GlobalScope.launch {
            broadcastPlayerSates()
            drawingPlayer?.socket?.send(gson.toJson(newWords))
    ...
    private fun showWord() {
        GlobalScope.launch {
            ...
            broadcastPlayerSates()
            word?.let {
                val choseWord = ChosenWord(it, name)
                broadcast(gson.toJson(choseWord))
            }
    ...
    private suspend fun broadcastPlayerSates() {
    ...
    private suspend fun sendWordToPlayer(player: Player) {
        ...
        word?.let { curWord ->
            drawingPlayer?.let { drawing ->
                ...
                player.socket.send(gson.toJson(gameState))
            }
        }
        player.socket.send(gson.toJson(phaseChange))
    }
